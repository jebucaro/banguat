...
{
    public sealed record RateResult(int Currency, string? CurrencyAlias, DateOnly? Date, decimal? Buy, decimal? Sell, int Count);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var result = await client.GetCurrentRateAsync(code, cancellationToken);
        if (result.IsFailure)
        {
        ...
        var point = result.Value.Rates[0];
