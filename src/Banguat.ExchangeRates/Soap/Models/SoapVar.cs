...
internal sealed record SoapVar(DateOnly Fecha, decimal Venta, decimal Compra)
{
    internal static SoapVar? FromElement(XElement element)
    {
        ...
        var venta = SoapXmlParsing.ParseDecimal(element.Element(BanguatSoapNamespaces.Service + "venta")?.Value);
        var compra = SoapXmlParsing.ParseDecimal(element.Element(BanguatSoapNamespaces.Service + "compra")?.Value);

        if (fecha is null || venta is null || compra is null) return null;
